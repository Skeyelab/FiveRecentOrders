(function() {
  return {
    ...
    events: {
      ...
      '*.changed': _.debounce(function(e){ this.fieldChanged(e); }, 500),
      'fetchData.done': 'fetchDataDone',
	'ticket.custom_field_{{email_address}}.changed' : 'redoSearch',
	'click .toggle-tbody': function(f){ this.$(f.currentTarget).parents('table').find('tbody').toggle(); },
	'click .autofilloi': function(g){ this.ticket().customField('custom_field_' + this.setting('order_id'), this.$(g.target).data('oi'));
...
redoSearch: function(){
    ...
    fetchDataFromApi: function(){
      ...
      if (email_address && country){
        this.ajax('fetchData', { email_address: email_address,
                                 country: country
                               });
